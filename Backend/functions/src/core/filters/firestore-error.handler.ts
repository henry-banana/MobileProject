      ...
      if (error && error.code) {
        ...
        if (code === 'FAILED_PRECONDITION') {
          if (
            ...
            const indexUrl =
              this.extractIndexUrl(error.message) ||
            ...
            if (isBuilding) {
              ...
              throw new HttpException(
                {
                  success: false,
                  message:
                    'Firestore index is building. Please retry in a few minutes or enable fallback mode for development.',
                  errorCode: 'FIRESTORE_INDEX_BUILDING',
                  details: {
                    firestoreMessage: error.message,
                    indexUrl,
                    suggestion:
                      'Set ENABLE_FIRESTORE_PAGINATION_FALLBACK=true in .env for development',
                  },
              ...
              this.logger.warn(`Firestore missing index: ${error.message}. Index URL: ${indexUrl}`);
        ...
        this.logger.error(`Firestore error [${code}]: ${message}`);
